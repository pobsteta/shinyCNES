...
check_gdal <- function(abort = TRUE, gdal_path = NULL, force = FALSE, full_scan = FALSE) {
  ...
  if (!is.null(gdal_path)) {
    tryCatch(
      gdal_path <- xfun::normalize_path(gdal_path, mustWork = TRUE),
      error = function(e) {
        print_message(
          type = message_type,
  ...
  if (any(
    ...
    force == TRUE,
    !grepl(gdal_path, xfun::normalize_path(nn(binpaths$gdalinfo)), fixed = TRUE)
  )) {
    # nocov start
    print_message(
      type="message",
    ...
    paths_gdalinfo <- xfun::normalize_path(paths_gdalinfo)
